Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/45b7c75ebfabcf944bd9217ab1522bf3"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)), float(1.05));
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.5149178,0.8148467,0.456411,1).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.5149178,0.8148467,0.456411,1).a;
